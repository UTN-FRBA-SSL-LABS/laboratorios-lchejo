...
> R:es para decir que empieza en cero y es importante porque puede que el carrito empieze con basuras osea valores viejos de memoria lo cual puede generar errores en los tests
...
> R:falla, esperaba 700 pero obtuvo 350
...
> R:linea 24 , y suma el productos

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:sumar el producto y multiplicar por la cantidad
...
> R:falla,[carrito lleno] [FAIL] resultado => esperado 0, obtenido 1  (linea 69)  Segmentation fault
...
> R:si las 29 y 30 (    #####:   29:int carrito_descuento(int total, int porcentaje) #####:   30:    return total - (total * porcentaje / 100);) , no se ejecuto porque solo se preuba en los tests de integracion no en los unitarios
...
> R:las unitarias spm árafunciones aisladas y las de integracion para prueba de interracciones, el unitario detecto primero el bug
...
> R:porque en C los arrays no tienen verificación de limites automatica. Si escribimos mas alla del array, sobrescribimos memoria 
...
> R:Los tests verifican comportamiento real, previenen regresiones y documentan el uso esperado
...
> R:porque 1 × precio = precio, el bug no afecta el resultado cuando cantidad = 1.
